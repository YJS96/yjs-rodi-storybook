...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground group-[.destructive]:focus:ring-destructive',
      className,
    )}
    {...props}
  />
...
>(({ children, className, ...props }, ref) => (
  <>
    <ToastPrimitives.Title ref={ref} className={cn(className)} {...props}>
      <Text as="h3" size="body-01" weight="medium" className="text-white">
        {children}
      </Text>
...
>(({ children, className, ...props }, ref) => (
  <ToastPrimitives.Description ref={ref} className={cn(className)} {...props}>
    {' '}
    <Text as="p" size="body-01" weight="regular" className="text-white">
      {children}
    </Text>
